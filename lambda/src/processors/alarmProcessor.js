...
async function getAllRelevantAlarms(cloudWatchClient, lambdaFunctionName, snsTopicArn) {
    ...
    try {
      ...
      const relevantAlarms = result.MetricAlarms.filter(alarm => {
        return alarm.AlarmActions && alarm.AlarmActions.some(action =>
          ...
          action.includes(snsTopicArn) // Also include alarms with SNS topics that might trigger our function
        );
      });
  ...
  // If no alarms found with function as action, get all alarms for demo purposes
  // In production, configure alarms to have the Lambda function as an action
